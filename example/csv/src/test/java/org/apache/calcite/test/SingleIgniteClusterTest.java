...
public class SingleIgniteClusterTest {
  //Ignite + Ignite 单 Schema（同集群）
  @Test
  public void testIgniteSingleSchemaJoin() throws Exception {
    // 设置字符集（避免中文乱码）
    System.setProperty("saffron.default.charset", "UTF-8");

    Properties info = new Properties();
    String modelPath = new File(
        getClass().getResource("/single-ignite.json").toURI()
    ).getAbsolutePath();
    ...
    try (Connection conn = DriverManager.getConnection("jdbc:calcite:", info)) {
      String sql = "SELECT " +
          ...
          "FROM ignite.CUSTOMER_INFO AS c " +
          "JOIN ignite.POSITION_LIVE AS p " +
      ...
      try (Statement stmt = conn.createStatement();
        ...
        System.out.println("--- Ignite 单集群 JOIN 查询结果 ---");
        while (rs.next()) {
